...
%% MAIN
% =========================================================================
% path to .mat containing all images
...
%% Functions
% =========================================================================
%% Returns the image with the bordering pixels continued
function [image, height, width] = read_image(image)
    % continue the image border
    image(1,:) = image(2,:);
    image(:,1) = image(:,2);
...
function [preprocessed_image] = preprocess_image(image)
    ...
    preprocessed_image = mat2gray(preprocessed_image);
    preprocessed_image = im2double(preprocessed_image);
end

%% Oversegmentation Function
function [mean_superpixels, label_matrix, idx, region_boundaries] = oversegment_image(image, height)
    ...
    [label_matrix, num_labels] = superpixels(image, height*3);
...
function [local_windows_dictionary] = determine_local_window(superpixel_centroids, roi, num_roi, idx, d)
    ...
    for label1 = 1 : num_roi
        ...
        for label2 = 1 : num_roi
            % if centroids within range of neighbours, add it as local window
